...
public final class CallInWebhookServer implements AutoCloseable {
    ...
    public CallInWebhookServer(AppConfig config) throws IOException {
        ...
        // Bind to loopback by default. Public exposure should terminate TLS at
        // an approved reverse proxy/tunnel and forward locally.
    ...
    private void handle(
        ...
        if(sessions.size()>500){
            AuditService.record("Rejected call-in webhook because session capacity was reached");
            respond(exchange,503,"Service Unavailable","text/plain");
            return;
        }
    ...
    private void expireOldSessions(){
        ...
        sessions.entrySet().removeIf(e->e.getValue().lastTouched<cutoff);
    }

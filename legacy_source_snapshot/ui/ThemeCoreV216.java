...
public final class ThemeCoreV216 {
    ...
    static void normalizeBeforePaint(Component component){
        ...
        try{
            ...
            standardizeEmployeeImmediately(component);
            modernizeTruckImmediately(component);
    ...
    private static void adaptActiveRoute(JFrame frame){
        ...
        try{
            ...
            removeLegacyRouteHeaders(frame);
            themeCombos(frame,Theme.active());
        } finally { NORMALIZING.set(Boolean.FALSE); }
    }

    private static void standardizeEmployeeImmediately(Component root){
        Component employee=findByClass(root,EMPLOYEE_CLASS);
    ...
    private static void modernizeTruckImmediately(Component root){
        Component truck=findByClass(root,TRUCK_CLASS);
        if(!(truck instanceof JPanel panel))return;
    ...
    private static boolean containsLegacyDashboardButton(Component c){
        ...
        if(c instanceof Container ct) for(Component child:ct.getComponents()) if(containsLegacyDashboardButton(child))return true;
        return false;
    }
    ...
    private static void polishDialog(JDialog dialog){
        ...
        root.putClientProperty("apple.awt.windowAppearance",theme.dark()?"NSAppearanceNameDarkAqua":"NSAppearanceNameAqua");
        root.setOpaque(true); root.setBackground(theme.bg()); root.setBorder(BorderFactory.createEmptyBorder());
        ...
        themeDialogTree(dialog,theme); themeCombos(dialog,theme);
        if(isTargetForm(title)){ boolean tracker=title.equalsIgnoreCase("Add NorthStar Tracker"); fit(dialog,tracker?620:900,tracker?560:650); }
        dialog.validate(); dialog.repaint();
    }
    private static boolean isTargetForm(String title){
        return title.equalsIgnoreCase("Add NorthStar Tracker")||title.equalsIgnoreCase("Add Network Location")||title.equalsIgnoreCase("Add Logistics Network Location");
    }
    private static void fit(JDialog dialog,int wantedW,int wantedH){
        ...
        int maxW=Math.max(440,b.width-i.left-i.right-40), maxH=Math.max(420,b.height-i.top-i.bottom-40);
        dialog.setMinimumSize(new Dimension(0,0)); dialog.setSize(Math.min(wantedW,maxW),Math.min(wantedH,maxH)); dialog.setLocationRelativeTo(dialog.getOwner());
    }
    private static void themeDialogTree(Component c,AppTheme theme){
        ...
        else if(c instanceof JPanel p){ p.setBackground(theme.bg()); if(p.getClass()==JPanel.class)p.setOpaque(true); }
        else if(c instanceof JViewport v){ v.setOpaque(true); v.setBackground(theme.bg()); }
    ...
    private static void themeCombos(Component c,AppTheme theme){
        ...
        if(c instanceof Container ct) for(Component child:ct.getComponents()) themeCombos(child,theme);
    }

    private static Component findByClass(Component c,String name){
        if(name.equals(c.getClass().getName()))return c;
        if(c instanceof Container ct) for(Component child:ct.getComponents()){ Component f=findByClass(child,name); if(f!=null)return f; }
        return null;
    }
    private static JTabbedPane findTabs(Component c){
        if(c instanceof JTabbedPane t)return t;
        if(c instanceof Container ct) for(Component child:ct.getComponents()){ JTabbedPane f=findTabs(child); if(f!=null)return f; }
        return null;
    }
    private static JFrame findWorkspaceFrame(Component c){
        Component p=c;
        while(p!=null){ if(p instanceof JFrame f&&isWorkspaceFrame(f))return f; p=p.getParent(); }
        Window w=SwingUtilities.getWindowAncestor(c); return w instanceof JFrame f&&isWorkspaceFrame(f)?f:null;
    }
    private static boolean isWorkspaceFrame(JFrame f){ return "com.wtm.ui.OperationsWorkspaceFrame".equals(f.getClass().getName()); }
    private static String activeRoute(JFrame f){
        try{ Field x=f.getClass().getDeclaredField("activeWorkspaceRoute"); x.setAccessible(true); Object v=x.get(f); return v==null?"":v.toString(); }
        catch(Exception ex){ return ""; }
    }
    private static Object invokePrivateStatic(String className,String method,Class<?>[] sig,Object... args){
        try{ Class<?> c=Class.forName(className); Method m=c.getDeclaredMethod(method,sig); m.setAccessible(true); return m.invoke(null,args); }

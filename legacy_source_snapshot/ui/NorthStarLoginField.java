...
public final class NorthStarLoginField extends JPanel {
    ...
    public NorthStarLoginField(Kind kind){
        ...
        /*
         * Treat the complete visual shell as the input target. The actual
         * editor is transparent and does not occupy every painted pixel, so
         * forwarding shell/icon clicks eliminates dead click zones.
         */
        MouseAdapter focusEditor = new MouseAdapter() {
            @Override public void mousePressed(MouseEvent e) {
                if (field.isEnabled()) {
                    /*
                     * Transfer focus immediately on mouse-down. requestFocus()
                     * is intentional here: the enclosing startup JFrame is a
                     * top-level focusable window, so this gives macOS a direct
                     * native focus handoff instead of relying solely on the
                     * asynchronous focus manager.
                     */
                    field.requestFocus();
        ...
        addMouseListener(focusEditor);
        icon.addMouseListener(focusEditor);
    ...
    public void setEnabled(boolean enabled){
        ...
        if(visibilityButton!=null) visibilityButton.setEnabled(enabled);
        repaint();
    }

...
public final class ConfigService {
    ...
    public static AppConfig load() {
        ...
        try {
            ...
            cfg.showWeatherAlertsInTicker = bool(p, "showWeatherAlertsInTicker", cfg.showWeatherAlertsInTicker);
            ...
            /*
             * v1.1.2: Workspace Setup supports up to 8 visible Information
             * metrics. The previous loader still clamped the persisted value
             * to 6, so selecting 8 appeared to work until the next launch.
             * Keep load/save/UI limits synchronized.
             */
            ...
            cfg.workspaceOpsVisibleCount=Math.max(2,Math.min(8,integer(p,"workspace.ops.count",8)));
            cfg.workspaceOpsMovementMode=p.getProperty("workspace.ops.movementMode","STATIC").trim().toUpperCase();
            if(!java.util.Set.of("STATIC","PAGED","TICKER").contains(cfg.workspaceOpsMovementMode)) cfg.workspaceOpsMovementMode="STATIC";
            cfg.workspaceOpsScrollSeconds=Math.max(5,Math.min(60,integer(p,"workspace.ops.scrollSeconds",10)));
            cfg.workspaceOpsTickerPixelsPerSecond=Math.max(8,Math.min(120,integer(p,"workspace.ops.tickerPixelsPerSecond",28)));

            cfg.dashboardPrimarySplitRatio=ratio(
                    p,"dashboard.layout.primaryRatio",cfg.dashboardPrimarySplitRatio);
            cfg.dashboardLowerSplitRatio=ratio(
                    p,"dashboard.layout.lowerRatio",cfg.dashboardLowerSplitRatio);
            cfg.dashboardWeatherSplitRatio=ratio(
                    p,"dashboard.layout.weatherRatio",cfg.dashboardWeatherSplitRatio);
            cfg.dashboardShowcaseSplitRatio=ratio(
                    p,"dashboard.layout.showcaseRatio",cfg.dashboardShowcaseSplitRatio);
            cfg.dashboardEventsSplitRatio=ratio(
                    p,"dashboard.layout.eventsRatio",cfg.dashboardEventsSplitRatio);
            cfg.dashboardWeatherHeightRatio=moduleHeightRatio(
                    p,"dashboard.layout.weatherHeightRatio",
                    cfg.dashboardWeatherHeightRatio);
            cfg.dashboardShowcaseHeightRatio=moduleHeightRatio(
                    p,"dashboard.layout.showcaseHeightRatio",
                    cfg.dashboardShowcaseHeightRatio);
            cfg.dashboardRightHeightRatio=moduleHeightRatio(
                    p,"dashboard.layout.rightHeightRatio",
                    cfg.dashboardRightHeightRatio);
            cfg.dashboardWeatherGrid=gridSpec(
                    p,"dashboard.grid.weather",cfg.dashboardWeatherGrid);
            cfg.dashboardShowcaseGrid=gridSpec(
                    p,"dashboard.grid.showcase",cfg.dashboardShowcaseGrid);
            cfg.dashboardEventsGrid=gridSpec(
                    p,"dashboard.grid.events",cfg.dashboardEventsGrid);
            cfg.dashboardCelebrationsGrid=gridSpec(
                    p,"dashboard.grid.celebrations",
                    cfg.dashboardCelebrationsGrid);
            cfg.dashboardInformationGrid=gridSpec(
                    p,"dashboard.grid.information",
                    cfg.dashboardInformationGrid);
            cfg.dashboardOperationsGrid=gridSpec(
                    p,"dashboard.grid.operations",
                    cfg.dashboardOperationsGrid);
            ...
            for(OperationsKpiConfig candidate:kpiDefaults.operationsKpis){
                boolean exists=cfg.operationsKpis.stream().anyMatch(
                        existing->existing.id().equalsIgnoreCase(candidate.id()));
                if(!exists&&candidate.dataSourceId()!=null
                        &&candidate.dataSourceId().startsWith("TRUCK_"))
                    cfg.operationsKpis.add(candidate);
            }
            ...
            cfg.showFedexTrucksOnMap=bool(
                    p,"truck.map.fedex",cfg.showFedexTrucksOnMap);
            cfg.showStarhubTrucksOnMap=bool(
                    p,"truck.map.starhub",cfg.showStarhubTrucksOnMap);
            cfg.truckDeliveredMapDays=Math.max(0,Math.min(30,
                    integer(p,"truck.map.deliveredDays",cfg.truckDeliveredMapDays)));
            cfg.truckCurrentHistoryDays=Math.max(0,Math.min(365,
                    integer(p,"truck.currentHistoryDays",cfg.truckCurrentHistoryDays)));
            cfg.truckArchiveAfterDays=Math.max(0,Math.min(3650,
                    integer(p,"truck.archiveAfterDays",cfg.truckArchiveAfterDays)));
            cfg.truckRetentionDays=Math.max(0,Math.min(36500,
                    integer(p,"truck.retentionDays",cfg.truckRetentionDays)));
            cfg.truckRefreshMinutes=Math.max(1,Math.min(1440,
                    integer(p,"truck.refreshMinutes",cfg.truckRefreshMinutes)));
            cfg.fedexTrackingMode=p.getProperty(
                    "truck.fedex.mode",cfg.fedexTrackingMode).trim();
            cfg.penskeTrackingMode=p.getProperty(
                    "truck.penske.mode",cfg.penskeTrackingMode).trim();
            cfg.penskeTrackingEndpoint=p.getProperty(
                    "truck.penske.endpoint",cfg.penskeTrackingEndpoint).trim();
            cfg.fedexEnvironment=p.getProperty(
                    "truck.fedex.environment",cfg.fedexEnvironment).trim();
            cfg.showTrak4OnMap=bool(p,"truck.trak4.map",cfg.showTrak4OnMap);
            cfg.trak4Mode=p.getProperty(
                    "truck.trak4.mode",cfg.trak4Mode).trim();
            cfg.trak4ReportsUrl=p.getProperty(
                    "truck.trak4.reportsUrl",cfg.trak4ReportsUrl).trim();
            cfg.trak4AuthHeader=p.getProperty(
                    "truck.trak4.authHeader",cfg.trak4AuthHeader).trim();
            cfg.trak4RefreshMinutes=Math.max(1,Math.min(1440,
                    integer(p,"truck.trak4.refreshMinutes",
                            cfg.trak4RefreshMinutes)));
            ...
            // loaded from encrypted credential store below. Keep the legacy TomTom
    ...
    public static void save(AppConfig cfg) {
        try {
            ...
            p.setProperty("showWeatherAlertsInTicker", Boolean.toString(cfg.showWeatherAlertsInTicker));
            ...
            p.setProperty("workspace.ops.count",Integer.toString(cfg.workspaceOpsVisibleCount));
            p.setProperty("workspace.ops.movementMode",cfg.workspaceOpsMovementMode);
            p.setProperty("workspace.ops.scrollSeconds",Integer.toString(cfg.workspaceOpsScrollSeconds));
            p.setProperty("workspace.ops.tickerPixelsPerSecond",Integer.toString(cfg.workspaceOpsTickerPixelsPerSecond));

            p.setProperty("dashboard.layout.primaryRatio",
                    Double.toString(cfg.dashboardPrimarySplitRatio));
            p.setProperty("dashboard.layout.lowerRatio",
                    Double.toString(cfg.dashboardLowerSplitRatio));
            p.setProperty("dashboard.layout.weatherRatio",
                    Double.toString(cfg.dashboardWeatherSplitRatio));
            p.setProperty("dashboard.layout.showcaseRatio",
                    Double.toString(cfg.dashboardShowcaseSplitRatio));
            p.setProperty("dashboard.layout.eventsRatio",
                    Double.toString(cfg.dashboardEventsSplitRatio));
            p.setProperty("dashboard.layout.weatherHeightRatio",
                    Double.toString(cfg.dashboardWeatherHeightRatio));
            p.setProperty("dashboard.layout.showcaseHeightRatio",
                    Double.toString(cfg.dashboardShowcaseHeightRatio));
            p.setProperty("dashboard.layout.rightHeightRatio",
                    Double.toString(cfg.dashboardRightHeightRatio));
            p.setProperty("dashboard.grid.weather",
                    cfg.dashboardWeatherGrid);
            p.setProperty("dashboard.grid.showcase",
                    cfg.dashboardShowcaseGrid);
            p.setProperty("dashboard.grid.events",
                    cfg.dashboardEventsGrid);
            p.setProperty("dashboard.grid.celebrations",
                    cfg.dashboardCelebrationsGrid);
            p.setProperty("dashboard.grid.information",
                    cfg.dashboardInformationGrid);
            p.setProperty("dashboard.grid.operations",
                    cfg.dashboardOperationsGrid);
            ...
            p.setProperty("truck.map.fedex",
                    Boolean.toString(cfg.showFedexTrucksOnMap));
            p.setProperty("truck.map.starhub",
                    Boolean.toString(cfg.showStarhubTrucksOnMap));
            p.setProperty("truck.map.deliveredDays",
                    Integer.toString(cfg.truckDeliveredMapDays));
            p.setProperty("truck.currentHistoryDays",
                    Integer.toString(cfg.truckCurrentHistoryDays));
            p.setProperty("truck.archiveAfterDays",
                    Integer.toString(cfg.truckArchiveAfterDays));
            p.setProperty("truck.retentionDays",
                    Integer.toString(cfg.truckRetentionDays));
            p.setProperty("truck.refreshMinutes",
                    Integer.toString(cfg.truckRefreshMinutes));
            p.setProperty("truck.fedex.mode",
                    cfg.fedexTrackingMode==null?"MANUAL":cfg.fedexTrackingMode);
            p.setProperty("truck.penske.mode",
                    cfg.penskeTrackingMode==null?"MANUAL_CSV":cfg.penskeTrackingMode);
            p.setProperty("truck.penske.endpoint",
                    cfg.penskeTrackingEndpoint==null?"":cfg.penskeTrackingEndpoint);
            p.setProperty("truck.fedex.environment",
                    cfg.fedexEnvironment==null?"PRODUCTION":cfg.fedexEnvironment);
            p.setProperty("truck.trak4.map",
                    Boolean.toString(cfg.showTrak4OnMap));
            p.setProperty("truck.trak4.mode",
                    cfg.trak4Mode==null?"MANUAL":cfg.trak4Mode);
            p.setProperty("truck.trak4.reportsUrl",
                    cfg.trak4ReportsUrl==null?"":cfg.trak4ReportsUrl);
            p.setProperty("truck.trak4.authHeader",
                    cfg.trak4AuthHeader==null?"Authorization":cfg.trak4AuthHeader);
            p.setProperty("truck.trak4.refreshMinutes",
                    Integer.toString(cfg.trak4RefreshMinutes));
             ...
             * leave a truncated properties file after an interrupted write.
    ...
    private static double ratio(Properties p,String key,double fallback){
        try{
            double value=Double.parseDouble(p.getProperty(
    ...
    private static double moduleHeightRatio(
            Properties p,String key,double fallback){
        try{
            double value=Double.parseDouble(p.getProperty(
                    key,Double.toString(fallback)));
            return Math.max(0.30,Math.min(1.00,value));
        }catch(Exception ex){
            return fallback;
        }
    }
    private static String gridSpec(
            Properties p,String key,String fallback){
        ...
        try{
            ...
            if(x<0||y<0||w<1||h<1||x+w>24||y+h>18){
                return fallback;
            }
            return x+","+y+","+w+","+h;
        }catch(Exception ex){

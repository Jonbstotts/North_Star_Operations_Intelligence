...
public final class AiEnabledMain {
    ...
    private static void injectSidebar(JFrame frame)throws Exception{
        ...
        int adminIndex=side.getComponentCount();
        for(int i=0;i<side.getComponentCount();i++){Component c=side.getComponent(i);if(c instanceof JLabel l&&"ADMINISTRATION".equalsIgnoreCase(l.getText())){adminIndex=i;break;}}
        int insertIndex=adminIndex;
        if(adminIndex>0){Component previous=side.getComponent(adminIndex-1);if(previous instanceof Box.Filler)insertIndex=adminIndex-1;}
        side.add(button,Math.max(0,insertIndex));side.revalidate();side.repaint();invokeSidebarSelection(frame);
    }
    ...
    private static void injectDashboard(JFrame frame)throws Exception{
        Field bodyField=frame.getClass().getDeclaredField("dashboardBody");bodyField.setAccessible(true);Object bodyObj=bodyField.get(frame);
        ...
        String saved=PREFS.get(GRID_KEY,"");
        String spec=validGridSpec(saved)?saved:findFreeSpec(grid,6,4);
        NorthStarIntelligenceCompactPanel compact=new NorthStarIntelligenceCompactPanel(()->openFull(frame));
        addTile.invoke(grid,"AI_INTELLIGENCE",compact,spec,5,4);
        grid.revalidate();grid.repaint();
        startGridPersistenceWatcher(grid,compact,spec);
    }

    private static void startGridPersistenceWatcher(Component grid,JComponent compact,String initial){
        final String[] last={initial};
        javax.swing.Timer watcher=new javax.swing.Timer(650,null);
        watcher.addActionListener(e->{
            if(!grid.isDisplayable()){
                watcher.stop();
                return;
            }
            try{
                String current=findTileGridSpec(grid,compact);
                if(current!=null&&!current.equals(last[0])){
                    PREFS.put(GRID_KEY,current);
                    last[0]=current;
                }
            }catch(Exception ignored){}
        });
        watcher.setInitialDelay(900);
        watcher.start();
    }

    private static String findTileGridSpec(Component grid,JComponent compact)throws Exception{
        Field tilesF=grid.getClass().getDeclaredField("tiles");tilesF.setAccessible(true);
        Object raw=tilesF.get(grid);
        if(raw instanceof Iterable<?> tiles){
            for(Object tile:tiles){
                Field contentF=tile.getClass().getDeclaredField("content");contentF.setAccessible(true);
                if(contentF.get(tile)!=compact)continue;
                Field rectF=tile.getClass().getDeclaredField("gridRect");rectF.setAccessible(true);
                Object rect=rectF.get(tile);
                int x=intField(rect,"x"),y=intField(rect,"y"),w=intField(rect,"w"),h=intField(rect,"h");
                return x+","+y+","+w+","+h;
            }
        }
        return null;
    ...
    private static boolean validGridSpec(String spec){
        if(spec==null||spec.isBlank())return false;
        try{
            String[] p=spec.split(",");if(p.length!=4)return false;
            int x=Integer.parseInt(p[0].trim()),y=Integer.parseInt(p[1].trim()),w=Integer.parseInt(p[2].trim()),h=Integer.parseInt(p[3].trim());
            return x>=0&&y>=0&&w>=5&&h>=4&&x+w<=24&&y+h<=18;

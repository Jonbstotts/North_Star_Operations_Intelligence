...
public final class AiEnabledMain {
    ...
    private static void injectSidebar(JFrame frame)throws Exception{
        ...
        // Insert before the section spacer that precedes ADMINISTRATION. This keeps
        // Intelligence in the OPERATIONS list with exactly the same vertical rhythm
        // as Sports/Main Showcase, while preserving the normal section gap below it.
    ...
    private static void injectDashboard(JFrame frame)throws Exception{Field bodyField=frame.getClass().getDeclaredField("dashboardBody");bodyField.setAccessible(true);Object bodyObj=bodyField.get(frame);if(!(bodyObj instanceof JPanel body)||!body.isShowing())return;if(findByName(body,"northstar.ai.compact")!=null)return;Component grid=findDashboardGrid(body);if(grid==null)return;Method addTile=grid.getClass().getDeclaredMethod("addTile",String.class,JComponent.class,String.class,int.class,int.class);addTile.setAccessible(true);String spec=findFreeSpec(grid,6,4);NorthStarIntelligenceCompactPanel compact=new NorthStarIntelligenceCompactPanel(()->openFull(frame));addTile.invoke(grid,"AI_INTELLIGENCE",compact,spec,5,4);grid.revalidate();grid.repaint();}

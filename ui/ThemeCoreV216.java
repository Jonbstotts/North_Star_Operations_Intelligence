...
public final class ThemeCoreV216 {
    ...
    private static void polishDialog(JDialog dialog){
        ...
        dialog.setBackground(theme.bg());themeDialogTree(dialog,theme);
    ...
    private static void themeDialogTree(Component c,AppTheme theme){
        ...
        if(c instanceof Container container)for(Component child:container.getComponents())themeDialogTree(child,theme);
    }
    private static void applyEmployeeFirstPaint(Component c,boolean inEmployees){
        ...
        if(employee&&c instanceof JSplitPane split){
            ...
            if(split.getOrientation()==JSplitPane.HORIZONTAL_SPLIT){
                ...
                if(left instanceof JComponent jc){jc.setMinimumSize(new Dimension(390,0));Dimension p=jc.getPreferredSize();jc.setPreferredSize(new Dimension(500,p==null?1:Math.max(1,p.height)));}
            }else split.setDividerSize(0);
        }
        if(c instanceof Container container)for(Component child:container.getComponents())applyEmployeeFirstPaint(child,employee);
    }
    private static void expandAppearanceScroll(Component c){
        if(c instanceof JScrollPane scroll){
            ...
            if(view instanceof JComponent jc&&(contains(view,"Startup Experience")||contains(view,"Appearance"))
                ...
                Dimension p=jc.getPreferredSize();int w=p==null?1:Math.max(1,p.width),h=p==null?1:Math.max(1,p.height);
                jc.setPreferredSize(new Dimension(w,h+850));
                Border old=jc.getBorder();jc.setBorder(BorderFactory.createCompoundBorder(old==null?BorderFactory.createEmptyBorder():old,new EmptyBorder(0,0,560,0)));
                scroll.getVerticalScrollBar().setUnitIncrement(24);scroll.getVerticalScrollBar().setBlockIncrement(200);scroll.revalidate();
            }
        }
        if(c instanceof Container container)for(Component child:container.getComponents())expandAppearanceScroll(child);
    }
    private static boolean contains(Component c,String text){
        ...
        if(c instanceof Container container)for(Component child:container.getComponents())if(contains(child,text))return true;
        return false;
    }
}

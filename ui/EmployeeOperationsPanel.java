...
public final class EmployeeOperationsPanel extends JPanel {
    ...
    private JComponent buildEmployeeDirectory(){
        ...
        JButton importCsv=new JButton("Import / Update CSV");
        importCsv.setToolTipText(
                "Bulk add or update employees by EmployeeNumber");
        importCsv.addActionListener(e->importEmployeesCsv());

        JButton template=new JButton("CSV Template");
        template.setToolTipText(
                "Save a starter employee CSV with supported columns");
        template.addActionListener(e->saveEmployeeCsvTemplate());
    ...
    private JComponent callInPanel(){
        ...
        JTextArea security=new JTextArea(
                ...
                +"exact public HTTPS URL. Do not expose the host computer "
    ...
    private void importEmployeesCsv(){
        JFileChooser chooser=new JFileChooser();
        chooser.setDialogTitle("Import / Update Employees from CSV");
        ...
        try{
            /*
             * Work against a copy first. A malformed row cannot partially
             * damage the persisted employee store; successful rows are saved
             * together only after the user reviews the import summary.
             */
            EmployeeStore.Snapshot candidate=snapshot.copy();
            ImportResult result=new EmployeeCsvImporter().importFile(
                    chooser.getSelectedFile().toPath(),candidate);

            int choice=JOptionPane.showConfirmDialog(
                    this,
                    result.summary()
                    +"\n\nApply these employee changes?\n"
                    +"Existing IDs, call-in PINs, training, attendance and "
                    +"performance history are preserved.",
            ...
            saveSnapshot();
            refreshEmployees();
            message("Employee CSV import applied.\n"+result.summary());
        }catch(Exception ex){
            error("CSV import failed: "+ex.getMessage());
        }
    }

    private void saveEmployeeCsvTemplate(){
        JFileChooser chooser=new JFileChooser();
        chooser.setDialogTitle("Save Employee CSV Template");
        chooser.setSelectedFile(new java.io.File("northstar_employee_import.csv"));
        ...
        try{
            Files.writeString(
                    file,
                    EmployeeCsvImporter.TEMPLATE_HEADER+System.lineSeparator()
                    +"10001,Example Employee,EXAMPLE,Operations,1st,"
                    +"2026-01-15,1/12,123-456-7890,,true,true,true,true,false"
                    +System.lineSeparator(),
                    StandardCharsets.UTF_8
            );
            message("Employee CSV template saved.");
        }catch(Exception ex){
            error("Unable to save CSV template: "+ex.getMessage());
        }
    }
